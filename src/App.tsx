...
const App: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(
          data.error || "The magic failed. The astral winds are too strong."
        );
      }
    ...
    <div className="min-h-screen flex flex-col bg-[#020617] text-slate-100 selection:bg-purple-500/30">
      ...
      <header className="py-6 px-8 border-b border-purple-900/30 bg-slate-950/80 backdrop-blur-md sticky top-0 z-40">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl md:text-2xl font-bold tracking-wider mystical-font text-purple-100">
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto p-4 md:p-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <section className="bg-slate-900/50 backdrop-blur-sm border border-slate-800 rounded-3xl p-6 shadow-xl">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-slate-950/50 p-4 rounded-2xl border border-slate-800/50">
                ...
                <p className="text-xl font-bold text-purple-200 mystical-font">
                  {personalRituals}
                </p>
              </div>
              <div className="bg-slate-950/50 p-4 rounded-2xl border border-slate-800/50">
          ...
          {/* Ritual Controls */}
          <section className="bg-slate-900/50 backdrop-blur-sm border border-slate-800 rounded-3xl p-6 shadow-xl">
            <h2 className="text-lg font-semibold mb-4 text-purple-200 mystical-font tracking-wide">
              Ritual Controls
            </h2>
            <div className="space-y-5">
              {/* Power Source */}
              <div>
                <label className="block text-[10px] font-medium text-slate-500 uppercase tracking-widest mb-3">
                  Power Source
                </label>
                <div className="grid grid-cols-1 gap-2">
                  <button
                    ...
                    className={`px-4 py-3 rounded-xl border text-xs transition-all text-left flex items-center justify-between ${
                      settings.model === "gemini-2.5-flash-image"
                        ? "border-purple-500 bg-purple-500/10 text-purple-100 shadow-[0_0_15px_rgba(168,85,247,0.05)]"
                  ...
                  <button
                    ...
                    className={`px-4 py-3 rounded-xl border text-xs transition-all flex items-center justify-between ${
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {(["1:1", "3:4", "4:3", "9:16", "16:9"] as AspectRatio[]).map(
                    (ratio) => (
                      <button
                        key={ratio}
          ...
          <section className="bg-slate-900/40 backdrop-blur-md border border-purple-500/10 rounded-3xl p-1 shadow-2xl ring-1 ring-white/5 overflow-hidden">
            <div className="p-8 space-y-6">
              ...
              <textarea
                ...
                className={`w-full h-32 bg-slate-950/50 border ${
                  error ? "border-red-500/30" : "border-slate-800"
                } rounded-2xl p-6 text-slate-100 focus:outline-none focus:border-purple-500/50 transition-all resize-none placeholder:text-slate-800 font-light leading-relaxed`}
              />
              <div className="flex justify-end pt-2">
                <button
                  onClick={handleGenerate}
                  disabled={isLoading}
                  className={`px-10 py-4 rounded-2xl font-bold transition-all shadow-xl flex items-center gap-3 group ${
                    isLoading
                      ? "bg-slate-800 text-slate-600"
                      : "bg-purple-600 hover:bg-purple-500 text-white hover:scale-[1.02] shadow-purple-600/10 active:scale-95"
                  }`}
                >
                  {isLoading ? (
                    <>
          ...
          <section className="bg-slate-900/50 backdrop-blur-sm border border-slate-800 rounded-3xl p-6 shadow-xl">
            ...
            {images.length === 0 && (
              <p className="text-xs text-slate-500">
                Your conjured images will appear here.
              </p>
            )}
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {images.map((img) => (
                <div
                  key={img.id}
                  className="group relative rounded-2xl overflow-hidden shadow-lg shadow-purple-800/30"
                >
                  <img
                    src={img.url}
                    alt={img.prompt}
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent opacity-0 group-hover:opacity-100 transition-opacity p-3 flex flex-col justify-end">
                    <p className="text-xs text-purple-300 truncate">{img.prompt}</p>
      ...
      {/* Footer */}
      <footer className="py-4 text-center text-purple-600 font-mono text-xs">
        &copy; {new Date().getFullYear()} The Witch's Grimoire
      </footer>
    </div>
  );
};
